...
| **移动端课表体验** | 面向 Android、iOS 和 HarmonyOS 手机设计，Web / PWA 用于开发、兼容和辅助分发。 |
...
| 平台 | 说明 |
| --- | --- |
| Android 手机 | 主要发布平台，安装包见 GitHub Releases；当前独占自动教务导入能力。 |
| iOS 手机 | 正式产品目标；当前支持本地构建，需要 macOS、Xcode 与签名环境，尚不支持自动教务导入。 |
| HarmonyOS 手机 | 正式产品目标；当前由实验性的 OpenHarmony WebView 工程承载。 |
| Web / PWA | 可在浏览器中使用，用于开发、兼容和辅助分发；部分原生教务导入能力不可用。 |
| PC / 桌面端 | 当前不作为发行或 UI 适配目标；后续宽屏优化优先面向 Pad。 |
